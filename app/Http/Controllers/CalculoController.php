...
class CalculoController extends Controller
{
    ...
    public function show($value='')
    {
    	...
    	$chats_posesion = Charts::create('pie', 'highcharts')
    	->title('Costos de posesión')
    	->labels(['Depreciación(D)', 'Intereses(I)', 'Seguros(S)'])
    	->values([$depreciacion_cal,$interes_cal,$seguros_cal])
    	->dimensions(1000,300)
    	->responsive(false);


    	$chats_operacion = Charts::create('pie', 'highcharts')
    	->title('Costos de Operación')
    	->labels(['Mantenimiento y Reparacion', 'Combustible', 'Filtros','Lubricantes','Grasas','Neumáticos','HH de operador'])
    	->values([$costo_mano_obra_rep,$comb,$filtros,$lubricantes,$gras,$neumaticos,$costo_hh])
    	->dimensions(1000,300)
    	->responsive(false);

    	$chats_horario = Charts::create('pie', 'highcharts')
    	->title('Costos Horario')
    	->labels(['Costos de Posesión', 'Costos de Operación'])
    	->values([$total_sin_operar,$total_operar])
    	->dimensions(1000,300)
    	->responsive(false);


    	return view('calculo.mostrar', compact('value',"prices","caracteristicas","operador","depreciacion_cal","interes_cal","seguros_cal","total_sin_operar",'costo_mano_obra_rep','comb','filtros','lubricantes','gras','neumaticos','costo_hh','total_operar','costo_horario','chats_posesion','chats_operacion','chats_horario'));
        }else{
